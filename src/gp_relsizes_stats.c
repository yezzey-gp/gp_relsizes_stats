...
static List *get_collectable_db_ids(List *ignored_db_names,
  ...
  if (retcode != SPI_OK_SELECT || SPI_processed < 0) {
    elog(ERROR,
         "get_collectable_db_ids: SPI_execute returned %d, processed %lu rows",
         retcode, SPI_processed);
    goto finish_SPI;
  }

  Datum *tuple_values =
      palloc0(SPI_tuptable->tupdesc->natts * sizeof(*tuple_values));
  bool *tuple_nullable =
      palloc0(SPI_tuptable->tupdesc->natts * sizeof(*tuple_nullable));

  for (int i = 0; i < SPI_processed; ++i) {
    HeapTuple current_tuple = SPI_tuptable->vals[i];
    heap_deform_tuple(current_tuple, SPI_tuptable->tupdesc, tuple_values,
                      tuple_nullable);
    ...
    if (!ignored) {
      MemoryContext old_context = MemoryContextSwitchTo(saved_context);
      collectable_db_ids =
          lappend_int(collectable_db_ids, DatumGetInt32(tuple_values[1]));
      MemoryContextSwitchTo(old_context);
    }
  }
...
static int create_truncate_fill_tables() {
  ...
  retcode = SPI_connect();
  if (retcode < 0) { /* error */
    elog(ERROR, "create_truncate_fill_tables: SPI_connect returned %d",
         retcode);
    retcode = -1;
    goto finish_SPI;
  }
  ...
  if (retcode != SPI_OK_UTILITY) {
    elog(ERROR,
         "create_truncate_fill_tables: SPI_execute returned %d (in create "
         "query)",
         retcode);
    retcode = -1;
    goto finish_SPI;
  ...
  if (retcode != SPI_OK_UTILITY) {
    elog(ERROR,
         "create_truncate_fill_tables: SPI_execute returned %d (in truncate "
         "query)",
         retcode);
    retcode = -1;
    goto finish_SPI;
  ...
  if (retcode != SPI_OK_INSERT) {
    elog(ERROR,
         "create_truncate_fill_tables: SPI_execute returned %d (in insert "
         "query)",
         retcode);
    retcode = -1;
    goto finish_SPI;
...
static void fill_file_sizes(int segment_id, char *data_dir,
  ...
  Tuplestorestate *tupstore =
      tuplestore_begin_heap(randomAccess, false, work_mem);
...
static int get_file_sizes_for_databases(List *databases_ids) {
  ...
  foreach (current_cell, databases_ids) {
    ...
    sprintf(
        query,
        "INSERT INTO gp_toolkit.segment_file_sizes (segment, relfilenode, filepath, size, mtime) \
    ...
    retcode = SPI_connect();
    if (retcode < 0) { /* error */
      elog(ERROR, "get_file_sizes_for_databases: SPI_connect returned %d",
           retcode);
      retcode = -1;
      goto finish_SPI;
    }
    ...
    /* check errors if they're occured during execution */
    if (retcode != SPI_OK_INSERT) {
      elog(ERROR, "get_file_sizes_for_databases: SPI_execute returned %d",
           retcode);
      retcode = -1;
      goto finish_SPI;
    }
...
static int write_final_result() {
  ...
  retcode = SPI_connect();
  if (retcode < 0) { /* error */
    elog(ERROR, "write_final_result: SPI_connect returned %d", retcode);
    retcode = -1;
    goto finish_SPI;
  }
  ...
  /* check errors if they're occured during execution */
  if (retcode != SPI_OK_UTILITY) {
    elog(ERROR, "write_final_result: SPI_execute returned %d (in create query)",
         retcode);
    retcode = -1;
    goto finish_SPI;
  }
...
Datum collect_table_size(PG_FUNCTION_ARGS) {
  ...
  get_typlenbyvalalign(elem_type, &elem_width, &elem_type_by_val,
                       &elem_alignment_code);
  deconstruct_array(ignored_db_names_array, elem_type, elem_width,
                    elem_type_by_val, elem_alignment_code, &args_datums,
                    &args_nulls, &args_count);
